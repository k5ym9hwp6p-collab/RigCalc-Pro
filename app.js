...
function renderHoles(){const tb=q("holeTable").querySelector("tbody");tb.innerHTML="";wpData.holes.sort((a,b)=>a.from-b.from).forEach((r,i)=>{const tr=document.createElement("tr");tr.innerHTML=`<td><input data-hole="${i}" data-k="from" type="number" value="${r.from}"></td><td><input data-hole="${i}" data-k="to" type="number" value="${r.to}"></td><td><input data-hole="${i}" data-k="id" type="number" value="${r.id}"></td><td><input data-hole="${i}" data-k="od" type="number" value="${r.od}"></td><td><button data-del-hole="${i}">✕</button></td>`;tb.appendChild(tr)})}
...
q("addHoleBtn").onclick=()=>{const last=wpData.holes[wpData.holes.length-1]||{to:0,id:171.45,od:127};wpData.holes.push({from:last.to,to:last.to+500,id:last.id,od:last.od});saveWP();renderWP()};
